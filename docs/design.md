...
**4. CLI provider** — the Kerberos chain entry gains a second mode: `GET https://cernbox.cern.ch/graph/v1.0/me` with `Authorization: Negotiate <AP-REQ>`, reading the Reva JWT back from the `x-access-token` response header written by the existing token writer. An endpoint dedicated to the exchange was built first and then removed: the token writer runs on every authenticated response, so such an endpoint returns a header the server already sends, and `/graph/v1.0/me` yields the identity in the same round trip.

**This is what shipped, and it is the only Kerberos path.** The design originally had a second mode that used the ticket to obtain an OIDC token from CERN SSO, with a toggle to choose between them, on the reasoning that the server side did not exist yet. It does now — implemented and tested against a real KDC — and keeping both would make a successful login depend on either CERNBox or auth.cern.ch being reachable: two things that can be down instead of one, and two code paths to keep working. The SSO bridge and its toggle were removed. §3.2 is kept below as a record of what was considered, not of what exists.
